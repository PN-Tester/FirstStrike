...
6. Run ```PCILeech.exe FirstStrike -kmd <AddressOfKmd> -0 0x55``` replacing <AddressOfKmd> with the address you received in the previous step.
...
# Warning
There are many many things that can go wrong during this process, and the tool has not undergone robust testing against various make and models of computer and windows operating systems. The tool has tested on windows 10, but only in a small sample size. Currently there are issues with windows 11 implementation that are being worked on. Because of the nature of shellcode placement in memory, its possible that various factors corrupt the injected shellcode and prevent successful detonation. This is ESPECIALLY TRUE IF YOU COMPILE THE KSH YOURSELF. I was having a lot of difficulty with corruption of the shellcode in memory and circumvented it by adding 
the NOP instructions at the tail-end of both the KMD and pscmd shellcodes in the patcher. This is a hacky fix. I pushed the corruption outside the range of usefull shellcode by doing this, but this doesnt prevent corruption from occuring. I will attempt to improve the reliability and stability of the injected payload when I have time.
...
# Troubleshooting
If you get get stuck on bootscreen loading forever after step 8, try repeating the process but change the ```-0``` parameter value at step 6 for something larger like ```-0 0x99```. This value controls the amount of thread skips that occur before the shellcode initialization.
