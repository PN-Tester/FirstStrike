...
VOID stage3_c_MainCommandLoop(PKMDDATA pk)
{
	...
	// 2: main dump loop
	while(TRUE) {
		if (idleCount == 300000000000) { // about 30 to 60 seconds after login screen
			//PNACEDIT
			//BYTE Source[] = { 0xbe, 0xef, 0xbe, 0xef, 0xbe, 0xef, 0xbe, 0xef };
			//pvMM = pk->fn.MmMapIoSpace(0x31cc289, 8, 0);
			//pk->fn.RtlCopyMemory(pvMM, Source, 8);
			((VOID(*)(PKMDDATA))pk->ReservedKMD[1])(pk); // EXEC WORKS HOLY FUCKAROOO
		}
		pk->_status = 1;
